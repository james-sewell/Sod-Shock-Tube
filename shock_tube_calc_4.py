...
def vector_a(u, nt, nx, gamma):
    ...
    for i in range(0, nt):
        ...
        u_2 = u[i][1]
        u_3 = u[i][2]
        a[i] = np.array([u_1, u_2 / u_1, (gamma - 1) * (u_3 - .5 * u_2 ** 2 / u_1),
...
def lax_wendroff(array, nt, nx, dt, dx, gamma):
    ...
    for i in range(0, nt - 1):
        ...
        u[i + 1][:, 1:-1] = un[:, 1:-1] - dt / dx * (f_star[:, 1:] - f_star[:, :-1])
        u[i + 1][:, 0] = un[:, 0]
        u[i + 1][:, -1] = un[:, -1]
...
plt.plot(case_a_exact[0], lax[nt_lax-1][3], 'x')
plt.xlabel('Position (m)')
plt.ylabel('Temperature (K)')
...
plt.figure(11)
plt.plot(case_a_exact[0], case_b_exact[4])
plt.plot(case_a_exact[0], god[nt_god-1][3], 'x')
plt.xlabel('Position (m)')
plt.ylabel('Temperature (K)')
plt.savefig('god_T')
